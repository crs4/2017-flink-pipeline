...
def make_parser():
    ...
    parser.add_argument('--samtools-path', help="Path to samtools executable.  If not set we look for it in the PATH")
...
def parse_args(args):
    ...
    if options.samtools_path:
        if not os.path.exists(options.samtools_path):
            p.error("Specified samtools doesn't exist")
        if not os.access(options.samtools_path, os.X_OK | os.R_OK):
            p.error("Specified samtools is not executable")
    else:
        options.samtools_path = get_exec('samtools')
...
def run_alignments(bcl_output_dir, output_dir, reference, bwa_path, samtools_path, nthreads):
    ...
    for name, file_lists in samples.iteritems():
        ...
        try:
            ...
            logger.exception(e)
            raise
        logger.info("Finished aligning %s.  Output file %s", name, output_file)
...
def main(args):
    ...
    try:
        ...
        #logger.info("Deleting input directory!! %s", options.input)
        #cmd = ['/bin/rm', '-rf', options.input]
